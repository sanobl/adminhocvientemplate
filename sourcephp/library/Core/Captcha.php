...
class Core_Captcha {
    ...
    function Core_Captcha($width = 200, $height = 30, $colorText = '#000000', $colorBg = '#ffffff') {
        $this->sImage = $this->FetchRegistrationString(6);
        $this->colorText = $colorText;
        $this->colorBg = $colorBg;
        $this->width = $width;
        $this->height = $height;
        $this->font = "./media/fonts/Roboto-Medium.ttf";
    }
    ...
    function show_image($text = '') {
        ...
        if (substr($this->colorBg, 0, 1) != '#') {
            ...
            switch ($imgbg_ext) {
                case "jpg" : $imbg = imagecreatefromjpeg("$bg");
                    break;
                case "epg" : $imbg = imagecreatefromjpeg("$bg");
                    break;
                case "gif" : $imbg = imagecreatefromgif("$bg");
                    break;
                case "png" : $imbg = imagecreatefrompng("$bg");
                    break;
            }
        ...
        /// Dance text		
            ...
            for ($ic = 0; $ic < strlen($this->sImage); $ic++) {
                imagefttext($im, $this->fontsize, rand(-45, 45), (($this->width / strlen($this->sImage)) * ($ic)) + 5, $this->height - (($this->height - $this->fontsize) / 2), $black, $this->font, $this->sImage{$ic}, array());
            }
        }
