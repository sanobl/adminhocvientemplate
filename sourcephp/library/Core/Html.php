...
class Core_Html
{
	public static function lazy($src, $alt='', $htmlOptions=array()){
		$htmlOptions['src']=Core_Global::getApplicationIni()->app->static->frontend->v2->images.'/grey.gif';
		$htmlOptions['alt']=$alt;
	 ...
	 * @since 1.0.5
	 */
	public static function renderAttributes($htmlOptions, $type='=')
	{
		...
		else
		{
			foreach($htmlOptions as $name=>$value)
			{
				if(isset($specialAttributes[$name]))
				{
					...
					$html .= ' ' . $name . $type . '"' . self::encode($value) . '"';
			}
		}
		return $html;
	}
